						...
						<div class="layui-input-block">
							<button type="button" class="layui-btn btn-theme" id="tupianUpload">
								<i class="layui-icon">&#xe67c;</i>上传图片
                                                                                ...
                                                                                <div class="layui-form-item" pane>
						<label class="layui-form-label">试卷文件：</label>
						<div class="layui-input-inline" style="line-height: 50px;">
							<input type="hidden" :value="detail.shijuanwenjian" id="shijuanwenjian" name="shijuanwenjian" />
                            <button type="button" class="layui-btn btn-theme" id="shijuanwenjianUpload">
								<i class="layui-icon">&#xe67c;</i>上传试卷文件
							</button>
							<a download="试卷文件" :href="detail.shijuanwenjian">点击下载试卷文件</a>
						</div>
					</div>
                                                                                <div class="layui-form-item" pane>
		...
		<script>
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
				...
				var tupianUpload = upload.render({
					...
					error: function() {
						...
						layer.msg("请求接口异常", {
							...
							icon: 5
												                ...
												                var shijuanwenjianUpload = upload.render({
					//绑定元素
					elem: '#shijuanwenjianUpload',
					//上传接口
					url: http.baseurl + 'file/upload',
					// 请求头
					headers: {
					...
					// 上传成功
						...
						if (res.code == 0) {
							...
							var url = http.baseurl + 'upload/' + res.file;
							jquery('#shijuanwenjian').val(url);
                            vue.detail.shijuanwenjian = url;
						} else {
							layer.msg(res.msg, {
								time: 2000,
								icon: 5
				...
				form.on('submit(*)', function(data) {
					...
					// 提交数据
					http.requestJson('shitixinxi' + '/add', 'post', data, function(res) {
					 	layer.msg('提交成功', {
					 		time: 2000,
					 		icon: 6
					 	}, function() {
